...
import math
import statistics
from typing import Callable, Optional, Tuple, Set, List, Union
import warnings
...
class AverageCorrelation:
    ...
    def __init__(
        ...
        macro: bool = True,
        p_filter_gold_ties_in_kendall_with_ties: float = 0,
        p_filter_gold_non_ties_in_kendall_with_ties: float = 0,
        n_resampling: int = 0,
        compute_human_ties_proportion: bool = False,
        use_sampled_pairs_only_to_compute_eps: bool = False,
        held_out_proportion: float = 0,
        held_out_row_ids: Set[int] = None,
        ...
        **corr_fcn_args,
        ...
        Args:
          ...
          p_filter_gold_ties_in_kendall_with_ties: Probability to filter out a gold tie
            in KendallWithTiesOpt.
          p_filter_gold_non_ties_in_kendall_with_ties: Probability to filter out a gold
            non-tie in KendallWithTiesOpt.
          n_resampling: Number of resampling to perform for each iteration. The corresponding
            acc_eq will be the avg across the several resampling (used only with KendallWithTiesOpt).
          compute_human_ties_proportion: Whether to compute human ties proportion
            (used only with KendallWithTiesOpt).
          use_sampled_pairs_only_to_compute_eps: Whether to use the sampled pairs only to compute the
                                                 optimal threshold (as a sort of alternative to the
                                                 'sample_rate' param).
          held_out_proportion: Proportion of the test data to hold out for computing the optimal threshold.
          held_out_row_ids: The row ids corresponding to the held-out. If passed, `held_out_proportion` will be ignored.
        ...
        self._macro = macro
        self._p_filter_gold_ties_in_kendall_with_ties = (
            p_filter_gold_ties_in_kendall_with_ties
        )
        self._p_filter_gold_non_ties_in_kendall_with_ties = (
            p_filter_gold_non_ties_in_kendall_with_ties
        )
        self._n_resampling = n_resampling
        self._compute_human_ties_proportion = compute_human_ties_proportion
        self._use_sampled_pairs_only_to_compute_eps = (
            use_sampled_pairs_only_to_compute_eps
        )
        self._held_out_proportion = held_out_proportion
        self._held_out_row_ids = held_out_row_ids
        self._return_std_dev = return_std_dev
        (
            self._pairwise_comparisons_list,
            self._human_ties_proportion,
            self._npairs,
            self._n_dev_rows,
            self._n_test_rows,
            self._test_set_human_ties_proportion,
    ...
    def __call__(
        ...
        gold_vect,
        metric_vect,
        return_best_threshold=False,
    ):
        """Same as standard correlation function, with one extra return value. If return_best_threshold is True,
        return the best threshold for KendallWithTiesOpt"""
        return self.Corr(
            gold_vect,
            metric_vect,
            return_best_threshold,
        )

    def Corr(
        self,
        gold_vect: ArrayLike,
        metric_vect: ArrayLike,
        return_best_threshold: bool = False,
    ):
        """Return correlation, pvalue, and denominator used for averaging. If return_best_threshold is True,
        return the best threshold for KendallWithTiesOpt."""
        ...
        if self._corr_fcn is KendallWithTiesOpt:
            if (
                self._p_filter_gold_ties_in_kendall_with_ties != 0
                or self._p_filter_gold_non_ties_in_kendall_with_ties != 0
                or self._held_out_proportion > 0
                or self._held_out_row_ids is not None
            ) and self._pairwise_comparisons_list is None:
                (
                    self._pairwise_comparisons_list,
                    self._human_ties_proportion,
                    self._held_out_row_ids,
                ) = self.get_pairwise_comparisons_list(
                    mat1,
                    self._n_resampling,
                    self._held_out_proportion,
                    held_out_row_ids=self._held_out_row_ids,
                )
                n_pairs_list = [len(pairs) for pairs in self._pairwise_comparisons_list]
                self._npairs = (
                    (statistics.mean(n_pairs_list), statistics.stdev(n_pairs_list))
                    if self._return_std_dev
                    else statistics.mean(n_pairs_list)
                )
            elif (
                self._human_ties_proportion is None
                and self._compute_human_ties_proportion
            ):
                assert self._pairwise_comparisons_list is None
                (
                    all_pairs,
                    self._human_ties_proportion,
                    _,
                ) = self.get_pairwise_comparisons_list(mat1)
                self._npairs = (
                    (len(all_pairs[0]), 0)
                    if self._return_std_dev
                    else len(all_pairs[0])
                )

            if self._held_out_proportion > 0:
                assert (
                    self._held_out_row_ids is not None
                    and self._pairwise_comparisons_list is not None
                )

            if self._held_out_row_ids is not None:
                test_set_human_ties_proportion = (
                    self.get_test_set_human_ties_proportion(mat1)
                )
                if self._test_set_human_ties_proportion is not None:
                    assert (
                        self._test_set_human_ties_proportion
                        == test_set_human_ties_proportion
                    )
                self._test_set_human_ties_proportion = test_set_human_ties_proportion

            corr, best_threshold, opt_result = KendallWithTiesOpt(
                gold_vect,
                metric_vect,
                num_sys=self._num_sys,
                average_by=self._average_by,
                pairwise_comparisons_list=self._pairwise_comparisons_list,
                use_sampled_pairs_only_to_compute_eps=self._use_sampled_pairs_only_to_compute_eps,
                use_held_out=self._held_out_row_ids is not None,
                **self._corr_fcn_args,
            )

            best_metric_ties, n_dev_rows, n_test_rows = (
                opt_result.best_metric_ties[0],
                opt_result.n_dev_rows[0],
                opt_result.n_test_rows[0],
            )
            if self._return_std_dev:
                corr, best_threshold, best_metric_ties, n_dev_rows, n_test_rows = (
                    opt_result.best_tau,
                    opt_result.best_threshold,
                    opt_result.best_metric_ties,
                    opt_result.n_dev_rows,
                    opt_result.n_test_rows,
                )

            if (
                not self._use_sampled_pairs_only_to_compute_eps
                and self._held_out_proportion == 0
                and self._held_out_row_ids is None
            ):
                assert n_dev_rows == n_test_rows

            if self._n_dev_rows is not None:
                assert (
                    self._n_dev_rows == n_dev_rows and self._n_test_rows == n_test_rows
                )
            else:
                self._n_dev_rows = n_dev_rows
                self._n_test_rows = n_test_rows

            result = [corr, 0, mat1.shape[0]]
            if return_best_threshold:
                result = [corr, best_threshold]
            if self._compute_human_ties_proportion:
                result.append(best_metric_ties)
            return tuple(result)
    ...
    def get_pairwise_comparisons_list(
        self,
        human_scores: np.ndarray,
        n_resampling: int = 1,
        held_out_proportion: float = 0,
        filter_nones: bool = True,
        held_out_row_ids: Optional[Set[int]] = None,
    ) -> Tuple[
        List[Set[Tuple[int, int, int]]], Tuple[float, float], Optional[Set[int]]
    ]:
        """Return the set of pairwise comparisons to consider for KendallWithTiesOpt."""
        # Convert the data to a numpy array in case it isn't already.
        human_scores = np.array(human_scores)
        if human_scores.ndim != 2:
            raise ValueError("human_scores must be a 2D array for KendallWithTiesOpt!")

        pairwise_comparisons_list, human_ties_proportions = [], []
        new_held_out_row_ids = []
        for _ in range(n_resampling):
            pairwise_comparisons, n_human_ties = [], 0
            for row_idx, row_human_scores in enumerate(human_scores):
                if held_out_row_ids is not None and row_idx not in held_out_row_ids:
                    continue
                ...
                if filter_nones:
                    filt = [
                        human_score_idx
                        for human_score_idx in range(len(row_human_scores))
                        if row_human_scores[human_score_idx] is not None
                    ]
                    if not filt:
                        continue
                    row_human_score_ids = filt
                else:
                    row_human_score_ids = range(len(row_human_scores))

                if held_out_row_ids is None and held_out_proportion > 0:
                    if held_out_proportion < np.random.random():
                        continue
                    new_held_out_row_ids.append(row_idx)

                for i in range(len(row_human_score_ids)):
                    for j in range(i + 1, len(row_human_score_ids)):
                        if self._p_filter_gold_ties_in_kendall_with_ties > 0.0:
                            if (
                                row_human_scores[row_human_score_ids[i]]
                                != row_human_scores[row_human_score_ids[j]]
                                or np.random.random()
                                > self._p_filter_gold_ties_in_kendall_with_ties
                            ):
                                n_human_ties += (
                                ...
                                pairwise_comparisons.append(
                                    (
                                        row_human_score_ids[i],
                                        row_human_score_ids[j],
                                        row_idx,
                                    )
                                )
                        elif (
            ...
            pairwise_comparisons_list.append(set(pairwise_comparisons))
            human_ties_proportions.append(n_human_ties / len(pairwise_comparisons))
            if held_out_row_ids is None and held_out_proportion > 0:
                assert len(new_held_out_row_ids) > 0
                held_out_row_ids = new_held_out_row_ids

        return (
            pairwise_comparisons_list,
            (
                statistics.mean(human_ties_proportions),
                statistics.stdev(human_ties_proportions)
                if len(human_ties_proportions) > 1
                else 0,
            )
            if self._return_std_dev
            else statistics.mean(human_ties_proportions),
            held_out_row_ids,
        )

    def get_test_set_human_ties_proportion(self, human_scores: np.ndarray) -> float:
        """Return the human ties proportion for the test set, excluding the held-out rows."""
        # Convert the data to a numpy array in case it isn't already.
        human_scores = np.array(human_scores)
        if human_scores.ndim != 2:
            raise ValueError("human_scores must be a 2D array for KendallWithTiesOpt!")

        n_pairs, n_human_ties = 0, 0
        for row_idx, row_human_scores in enumerate(human_scores):
            if row_idx in self._held_out_row_ids:
                continue
    ...
    def get_held_out_row_ids(self) -> Optional[Set[int]]:
        return self._held_out_row_ids
...
def KendallWithTiesOpt(
    ...
    opt_result = tau_optimization.tau_optimization(
        ...
        gold,
        tau_fn,
        pairwise_comparisons_list=pairwise_comparisons_list,
        use_sampled_pairs_only_to_compute_eps=use_sampled_pairs_only_to_compute_eps,
        use_held_out=use_held_out,
        sample_rate=sample_rate,
    )
    return opt_result.best_tau[0], opt_result.best_threshold[0], opt_result
