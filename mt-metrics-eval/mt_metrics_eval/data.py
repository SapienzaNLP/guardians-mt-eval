...
import sys
import tarfile
from typing import Callable, Dict, Iterable, List, Sequence, Set, Tuple, Union, Any
...
class EvalSet:
    ...
    def _ReadDataset(self, name, lp, read_stored_metric_scores, path, strict):
        ...
        self._scores = {}
        for filename in glob.glob(os.path.join(d, "human-scores", "%s.*.score" % lp)):
            lp, scorer, level = self.ParseHumanScoreFilename(os.path.basename(filename))
...
def GetCorrelations(
    ...
    for metric_name in evs.metric_names:
        ...
        if (
            primary_metrics
            and evs.primary_metrics
            and base_name not in evs.primary_metrics
        ):
            continue
...
def CompareMetrics(
    ...
    return_metric_name2best_tie_threshold: bool = False,
    return_human_ties_proportion: bool = False,
    ...
    return_n_rows: bool = False,
    **corr_fcn_args,
) -> Tuple[
    Union[
        Union[Dict[str, List[Union[int, Any]]], ndarray, ndarray, Dict[str, float]], Any
    ],
    ...,
]:
    ...
    Args:
      ...
      macro: If True, correlations are computed with a plain average over row- or
        item-wise correlations. If False, then if corr_fcn returns three values,
        the 3rd is used to weight each sys- or item-wise correlation, otherwise the
      ...
      parallel_file: If not None, the significance matrix will be computed in
        ...
        beam output.
      return_metric_name2best_tie_threshold (bool): If True, return a dict
        mapping metric names to the best tie threshold for that metric. This
        is used only when corr_fcn is stats.KendallWithTiesOpt.
      return_human_ties_proportion: If True, return the proportion of human ties.
      ...
      use_sampled_pairs_only_to_compute_eps: Whether to use the sampled pairs only to compute the
                                             ...
                                             'sample_rate' param).
      held_out_proportion: If > 0, it defines the proportion of the test set that will be used as held-out for ties.
    ...
    Returns:
      ...
      4. Draws list: an K x 2 numpy array of resampling results. draws_list[k]
         ...
         by draws_index.
      5. Mapping from metric name to best tie threshold. Returned iff
         return_metric_name2best_tie_threshold input parameter is True.
      6. Human ties proportion. Returned iff return_human_ties_proportion
         input parameter is True.
      7. Mapping from metric name to number of ties. Returned iff
         return_human_ties_proportion input parameter is True.
    """
    ...
    corr_wrapper = stats.AverageCorrelation(
        ...
        macro=macro,
        p_filter_gold_ties_in_kendall_with_ties=p_filter_gold_ties_in_kendall_with_ties,
        p_filter_gold_non_ties_in_kendall_with_ties=p_filter_gold_non_ties_in_kendall_with_ties,
        n_resampling=n_resampling,
        compute_human_ties_proportion=return_human_ties_proportion,
        use_sampled_pairs_only_to_compute_eps=use_sampled_pairs_only_to_compute_eps,
        held_out_proportion=held_out_proportion,
        held_out_row_ids=held_out_row_ids,
    ...
    corrs_and_ranks = {}
    # Used iff corr_fcn is stats.KendallWithTiesOpt and return_metric_name2best_tie_threshold is True.
    metric_name2best_tie_threshold = dict()
    # Used iff corr_fcn is stats.KendallWithTiesOpt and return_human_ties_proportion is True.
    metric_name2n_ties = dict()
    # Used iff corr_fcn is stats.KendallWithTiesOpt and return_std_dev is True.
    metric_name2tau_with_std_dev = dict()
    for m, c in metric_corrs.items():
        corr_wrapper_out = corr_wrapper(
            c.gold_scores,
            c.metric_scores,
            return_metric_name2best_tie_threshold,
        )
        corrs_and_ranks[m] = [
            corr_wrapper_out[0] if not return_std_dev else corr_wrapper_out[0][0],
            0,
        ]

        if corr_fcn == stats.KendallWithTiesOpt:
            if return_metric_name2best_tie_threshold:
                metric_name2best_tie_threshold[m] = corr_wrapper_out[1]

            if return_human_ties_proportion:
                metric_name2n_ties[m] = corr_wrapper_out[-1]

            if return_std_dev:
                metric_name2tau_with_std_dev[m] = corr_wrapper_out[0]
    ...
    if len(metric_name2best_tie_threshold) > 0:
        metric_name2best_tie_threshold = {
            metric_name: metric_name2best_tie_threshold[metric_name]
            for metric_name in corrs_and_ranks
        }

    if len(metric_name2n_ties) > 0:
        metric_name2n_ties = {
            metric_name: metric_name2n_ties[metric_name]
            for metric_name in corrs_and_ranks
        }

    if len(metric_name2tau_with_std_dev) > 0:
        metric_name2tau_with_std_dev = {
            metric_name: metric_name2tau_with_std_dev[metric_name]
            for metric_name in corrs_and_ranks
        }
    ...
    results_to_return = [corrs_and_ranks, sig_matrix, draws_index, draws_list]

    if return_metric_name2best_tie_threshold:
        results_to_return.append(metric_name2best_tie_threshold)

    if return_human_ties_proportion:
        results_to_return.append(human_ties_proportion)
        results_to_return.append(n_pairs)
        results_to_return.append(metric_name2n_ties)

    if return_n_rows:
        results_to_return.append(n_dev_rows)
        results_to_return.append(n_test_rows)

    if held_out_row_ids is None and held_out_proportion > 0:
        held_out_row_ids = corr_wrapper.get_held_out_row_ids()
        if len(held_out_row_ids) == 0:
            raise ValueError(
                "No rows were present in the held out, and held_out_proportion > 0!"
            )
        results_to_return.append(held_out_row_ids)
        results_to_return.append(test_set_human_ties_proportion)

    if return_std_dev:
        results_to_return.append(metric_name2tau_with_std_dev)
...
def CompareMetricsWithGlobalAccuracy(
    ...
    for evs, main_refs, close_refs, gold, extern_metrics in zip(
        evs_list, main_refs_list, close_refs_list, gold_name, extern_metrics_list
    ):
        corrs.append(
            GetCorrelations(
                evs,
    ...
    def ComputePval(
        ...
        if perm_test == "scores":
            pval, _, _, draws = stats.PermutationSigDiff(
                metric_corrs[metric2],
                metric_corrs[metric1],
                corr_fcn,
                average_by,
                k,
                psd,
...
def PrintMetricComparison(
    ...
    file=sys.stdout,
    probs=False,
    print_also_in_tsv_and_latex_formats=False,
):
    ...
    if print_also_in_tsv_and_latex_formats:
        print("\n")
        # tsv format
        for metric, (score, rank) in ranks.items():
            metric = evs.DisplayName(metric) if evs else metric
            print(f"{metric}\t{rank}\t{score}", file=file)

        print("\n")
        # LaTeX format
        print("\\begin{tabular}{lcc}", file=file)
        print("\\hline", file=file)
        print("Metric & Rank & Correlation \\\\", file=file)
        print("\\hline", file=file)
        for metric, (score, rank) in ranks.items():
            metric = evs.DisplayName(metric) if evs else metric
            # Escape underscores in metric names for LaTeX
            metric = metric.replace("_", "\\_")
            print(f"{metric} & {rank} & {score:.7f} \\\\", file=file)
        print("\\hline", file=file)
